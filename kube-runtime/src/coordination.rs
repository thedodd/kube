...
//! However, before taking leadership actions, clients can use the `fence` function to
//! ensure that the lease is still held. As long as all clients behave according to this
//! protocol, then a strong degree of fencing can be achieved.

/* TODO:
- Update the above docstring, particularly the last paragraph.
- Audit all tracing, use skip_all, and ensuring debugging is good.
*/
...
pub struct ConfigBuilder {
    ...
    #[allow(dead_code)] // TODO: use the retry period.
...
impl LeaderElector {
    ...
    fn update_state(&mut self, lease_opt: Option<Lease>) {
        ...
        let updated_lease = match lease_opt {
            Some(lease) => {
